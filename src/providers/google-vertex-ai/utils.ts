...
export const GoogleToOpenAIFinetune = (response: GoogleFinetuneRecord) => {
    return {
        ...
        hyperparameters: {
            batch_size:
                response.supervisedTuningSpec.hyperParameters?.adapterSize,
            learning_rate_multiplier:
                response.supervisedTuningSpec.hyperParameters
...
export const generateSignedURL = async (
    ...
    const escapedObjectName = encodeURIComponent(objectName).replace(
        /%2F/g,
        "/",
    );
...
export const transformGoogleTools = (tool: Tool) => {
    ...
    if (["googleSearch", "google_search"].includes(tool.function.name)) {
        ...
        ["codeExecution", "code_execution"].includes(tool.function.name)
    ) {
        tools.push({
            codeExecution: {},
        });
