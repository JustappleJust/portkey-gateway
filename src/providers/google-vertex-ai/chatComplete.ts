                ...
                params.messages?.forEach((message: Message) => {
                    ...
                    if (message.role === "assistant" && message.tool_calls) {
                        ...
                        parts.push({
                            functionResponse: {
                                name:
                                    message.name ?? "gateway-tool-filler-name",
                                response: {
                        ...
                        message.content.forEach((c: ContentType) => {
                            if (c.type === "text") {
                                ...
                                const { url, mime_type: passedMimeType } =
                                    c.image_url || {};
...
export const GoogleChatCompleteResponseTransform: (
    ...
    if (isValidResponse) {
        ...
        const outputAudioTokens = candidatesTokensDetails.reduce(
            (acc, curr) => {
                if (curr.modality === VERTEX_MODALITY.AUDIO)
                    return acc + curr.tokenCount;
                return acc;
